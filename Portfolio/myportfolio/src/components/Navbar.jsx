...
const Navbar = () => {
  ...
  return (
    <nav
      className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 ${scrolled
        ? "bg-primary"
        : "bg-transparent"}`}
    >
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-white text-[18px] font-bold cursor-pointer flex ">
            Hamza &nbsp;
            <span className="sm:block hidden"> | Khan-Benjamin</span>
          </p>
        </Link>
          ...
          >
            <ul className="list-none flex justify-end items-start flex-1 flex-col gap-4">
              {navLinks.map(nav =>
                <li
                  key={nav.id}
                  className={`font-poppins font-medium cursor-pointer text-[16px] ${active ===
                  nav.title
                    ? "text-white"
                    : "text-secondary"}`}
                  onClick={() => {
